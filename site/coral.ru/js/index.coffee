...
ASAP ->
    ...
    $.when($flickityReady).done ->
        $dots = $('.progress-dash').on 'click', (e) -> $slider.flickity 'select', $(this).index()
        $slider = $('#hotels-of-the-week .hotels-slider')
        .on 'select.flickity', (e, idx) ->
            $dots.eq(idx).addClass('is-selected').siblings('.is-selected').removeClass('is-selected')
        .flickity
